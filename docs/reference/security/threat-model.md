...
| **Lifecycle-header CORS visibility** | For an already-allowed browser origin, `Access-Control-Expose-Headers` permits JavaScript to read `Deprecation`, `Sunset`, and `Link` when a route emits them. A foreign Origin receives no CORS grant, so this does not make lifecycle metadata readable cross-origin. | Response visibility only; it does not broaden the loopback bind, Host allowlist, MCP Origin check, mutation token, or CSP. |
