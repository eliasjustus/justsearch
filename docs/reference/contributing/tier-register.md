...
| `hook` | PreToolUse / PostToolUse **blocking** hooks (`repeat-guard.mjs`, `build-counter.mjs`, `intervene.mjs`, `subagent-model-guard.mjs`) | ~100% |
| `hook-hint` | PreToolUse / PostToolUse **non-blocking** hint hook — *delivers* a rule at its moment of relevance (raises salience), does not block (`compact-restore`, `agent-spawn-build-hint`) | ~85% |
| `harness` | The Claude Code harness refuses the action itself via `.claude/settings.json` `permissions.deny` — no project code runs, so there is no `Resolves to` marker | ~100% |
...
| 2 | `loopback-only-network` | Local API trust boundary — loopback bind, Host allowlist, mutation token ([ADR-0046](../../decisions/0046-local-api-trust-boundary.md)) | `archunit` | `archunit:UiApiGuardrailsTest` | `LocalApiServer` binding policy + UI API guardrails test; the trust boundary's other controls are pinned by ADR-0046's premise probes (`adr-coverage` gate) and by `ApiSecurityFiltersTest` / `LocalApiHostValidationTest` / `McpOriginValidationTest` |
...
| 15 | `log-pre-existing-issues` | Route Out-of-Scope Findings, Don't Log Them — fix small doc drift in place; otherwise route to pin / rules / owning tempdoc (tempdoc 872) | `prose-only` | — | Agent discipline |
...
| 18 | `delegating-to-subagents` | Delegating to Subagents — brief inline, no destructive git delegation | `prose-only` | — | Brief-inline and no-destructive-git stay agent discipline. The **model-routing** subset is hook-enforced by `subagent-model-guard.mjs` (PreToolUse/`Agent`: blocks unpinned + fable spawns, incl. *nested* ones — the live-verified exception to row 28). Uncovered: fork-type subagents (ignore `model`, claude-code#74788) and the dispatch-picker path (#64493) |
| 32 | `tempdocs-are-dated-history` | Tempdocs Are Dated History, Not Current Truth — check highest-numbered tempdoc + frontmatter, verify against `main` before trusting | `prose-only` | — | Agent discipline; the rule's own instruction (check frontmatter + the highest-numbered tempdoc) is the check. A PostToolUse/Read age-stamp hook delivered this until tempdoc 930 row 4 retired the unmeasured hint tier (it fired 154 times in 30 days with no measurable effect) |
| 43 | `retire-with-a-sweep` | Retire With a Sweep — a retiring PR sweeps the retiree's fingerprints (code/config/gates/baselines/ignores/docs) in the same PR | `prose-only` | — | Agent discipline; "a retirement happened" has no mechanical detector — the gate-input contract (742) covers the gate subclass, prose the rest |

## .claude/rules/branch-safety.md

| # | Slug | Rule | Tier | Resolves to | Catches violations via |
|---|---|---|---|---|---|
| 19 | `never-checkout-in-main` | Never `git checkout` in the main worktree | `prose-only` | — | Agent discipline. A PreToolUse guard blocked this until tempdoc 930 row 4 retired it: measured over 30 days the destructive-git rules had 0 true positives and 11 false ones (worktree-legitimate `checkout --detach` / `checkout --ours` after a `cd` inside a compound command), and `permissions.deny` cannot be scoped to "main worktree only", so a native rule would be strictly worse |
...
| 23 | `never-destructive-git-in-main` | Never run destructive git in main worktree | `prose-only` | — | Agent discipline. Same retirement as row 19 (tempdoc 930 row 4): zero destructive commands in the main checkout were intercepted in 30 days, and the blocks that did fire were worktree-legitimate `reset --hard` after a `cd` |
...
| 25 | `never-force-push` | Never `git push --force` to main/master | `harness` | — | `.claude/settings.json` `permissions.deny` carries `Bash(git push --force*)` and `Bash(git push -f*)`. Tempdoc 930 F1 probed the semantics: deny is enforced under `defaultMode: bypassPermissions`, needs no restart, is evaluated per segment of a compound command, and matches prefix-only — so it keeps the block without the retired guard's false positives (its regex spanned `&&` into a later `gh workflow run -f`). Not covered: the refspec form `git push origin +main:main`, which no prefix rule can express |
...
| 36 | `docs-ride-along` | Publishing docs-only changes: tempdoc/observations edits ride-along with their code PR or batch; canonical-doc updates may stand alone (ADR-0045 axis-2 / tempdoc 653) | `prose-only` | — | Reviewer catches it at PR time. A PreToolUse `git push` hint delivered this until tempdoc 930 row 4 retired it — it is the one hint that was ever *measured* (739 §2) and the measurement was negative: it misfired on non-pushes, diffed the wrong repo, lost to a competing instruction, and even after the single-file threshold narrowed it (owner decision 2026-07-15, 739 §6) it fired in only 12 of 120 sessions |
...
| 40 | `squash-merge-verify-content-not-ancestry` | Don't conclude squash-merged work is "unmerged" from `git log`/branch-ancestry alone — diff content against `main` instead (tempdoc 727 F-8a) | `prose-only` | — | Agent discipline; the check is a single command, no automation is warranted — a gate/hook can't tell "content already landed" from "genuinely new" without the same diff a human/agent would run |
| 45 | `agent-spawn-session-end-reap` | Agent-spawned helper processes (ui-shot's Vite, `serve-worktree-fe`) are reaped automatically at session start/end/teardown via the `agent-spawns/` registry — never hand-`taskkill` one (tempdoc 861) | `hook-hint` | `hook:agent-spawn-session-end-reap.mjs` | `agent-spawn-session-end-reap.mjs` (SessionEnd) best-effort reaps this session's own registered spawns; `agent-spawn-sweep-hint.mjs` (SessionStart, async) covers a prior session's abandoned ones and `remove-worktree.cjs` covers teardown — a non-blocking delivery assist (~85%); the reaper's identity re-verification (861 §6.2/§6.3) is what a hand `taskkill` would bypass |
| 46 | `agent-spawn-build-hint` | A registered agent-spawn holding a path under a build's target tree is named before the build fails, never killed automatically (861 [A4]) | `hook-hint` | `hook:agent-spawn-build-hint.mjs` | `agent-spawn-build-hint.mjs` (PreToolUse Bash, gradlew/npm-shaped commands) names the holder + a ready-to-run remedy — advisory only, structurally unable to obtain a kill list ([A4]: `before-a-build` binds to `capability:'advisory'` in the reaper's frozen `OCCASIONS` map) |

## .claude/rules/agent-lessons.md

| # | Slug | Rule | Tier | Resolves to | Catches violations via |
|---|---|---|---|---|---|
| 27 | `subagents-no-inheritance` | Subagent CLAUDE.md inheritance is agent-type-dependent (gp/custom/fork: full; Explore/Plan: none — hook brief only); task-specific brief inline always | `prose-only` | — | Platform behavior (re-verified 2026-07-16, tempdoc 743 probes; supersedes the pre-2026-07 "no inheritance" claim). The `subagent-guide` SubagentStart hook injects the Hard-Invariants baseline for all types (sole carrier for Explore/Plan); the task-specific brief is honor-system |
...
| 37 | `piped-exit-masked` | A build/test command piped into `tail`/`grep`/`head` reports the pipe's exit code, not its own — a red build can read as green (tempdoc 618 §10a) | `prose-only` | — | The always-loaded §10a bullet in `agent-lessons.md` is now the whole enforcement; a PreToolUse/Bash hint delivered it at the moment of relevance until tempdoc 930 row 4 retired the unmeasured hint tier |
| 41 | `edit-reread-cross-root` | A worktree-copy and main-checkout copy of the "same" file don't share `Edit`'s read-state — re-read the exact path, not just "a" copy (tempdoc 618 §11e, closed by 727 F-7a) | `prose-only` | — | The `agent-lessons.md` `edit-reread-cross-root` bullet is now the whole enforcement; the Edit tool's own "has not been read" failure is the concrete signal. A `PostToolUseFailure` hint named the other same-basename path until tempdoc 930 row 4 retired the unmeasured hint tier |
| 42 | `subset-isnt-the-suite` | A hand-picked subset of gates/tests passing is not "the gates passed" — run the full kernel + full suite before declaring done, not at merge (tempdoc 618 §10c) | `prose-only` | — | The `agent-lessons.md` Named-substrate-discipline-principles bullet is now the whole enforcement, with postmortem #13 as the worked case. A PostToolUse/Bash hint fired it at `git merge` until tempdoc 930 row 4 retired the unmeasured hint tier |
| 44 | `utf8-bulk-edits` | Windows bulk edits corrupt UTF-8 via cp1252 round-trips — briefs mandate UTF-8-safe tooling; orchestrator diff-checks for unintended non-ASCII (tempdoc 742) | `prose-only` | — | Platform constraint; parent hooks don't fire in subagents (row 28) and bulk edits are subagent work — brief + orchestrator diff check is the only control surface |
