...
| 36 | `docs-ride-along` | Publishing docs-only changes: tempdoc/observations edits ride-along with their code PR or batch; canonical-doc updates may stand alone (ADR-0045 axis-2 / tempdoc 653) | `hook-hint` | `hook:docs-granularity-hint.mjs` | `docs-granularity-hint.mjs` (PreToolUse Bash `git push`) fires when a branch's whole diff vs `origin/main` is **exactly one** `docs/tempdocs/**` / `docs/observations*` file, delivering the ride-along/batch convention — non-blocking (~85%); canonical-doc-only, docs+code, and **multi-file archaeology batches** (a shard fold, several tempdoc edits) intentionally don't trigger. The single-file threshold is owner decision 2026-07-15 (tempdoc 739 §6): firing on every archaeology-only branch was right on only 22 of 54 (41%) measured on `main`, so it nagged the batches the rule permits and trained its own discounting |
...
| 47 | `falsify-restore-from-backup` | A break-and-restore falsification driver must restore from a byte backup, assert its break actually matched, and wipe test-results per case — `git checkout -- <file>` reverts to HEAD and silently deletes the uncommitted work being falsified (tempdoc 915 round 3) | `prose-only` | — | Platform constraint on ad-hoc driver scripts, which are written per-campaign and leave no durable artefact to gate; the always-loaded line in `agent-lessons.md` is the whole control surface |
...
| 41 | `edit-reread-cross-root` | A worktree-copy and main-checkout copy of the "same" file don't share `Edit`'s read-state — re-read the exact path, not just "a" copy (tempdoc 618 §11e, closed by 727 F-7a) | `hook-hint` | `hook:edit-reread-hint.mjs` | `edit-reread-hint.mjs` (`PostToolUseFailure`/Edit) fires on a "has not been read"/"modified since read" failure and names the other same-basename path already read this session, when one exists — a non-blocking delivery assist (~85%); the agent-lessons.md prose is the fallback for the silent case |
| 42 | `subset-isnt-the-suite` | A hand-picked subset of gates/tests passing is not "the gates passed" — run the full kernel + full suite before declaring done, not at merge (tempdoc 618 §10c) | `hook-hint` | `hook:merge-full-suite-hint.mjs` | `merge-full-suite-hint.mjs` (PostToolUse Bash `git merge`, any form except `--abort`) fires a full-suite reminder citing the concrete tempdoc 734 instance (a two-consecutive-reads rebuild-finalize guard vs. a call-it-once shutdown path, merged clean with no conflict marker, caught only by the full suite) — a non-blocking delivery assist (~85%); the agent-lessons.md Named-substrate-discipline-principles bullet stays as the fallback for a checkout without wired hooks |
