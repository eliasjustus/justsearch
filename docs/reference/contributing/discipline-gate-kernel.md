...
| `todo-fixme` | Three surfaces, all wired in CI's `Public claims` job. **ui-web**: ESLint `no-warning-comments` in `modules/ui-web/eslint.config.js` (step "Frontend lint", `npm --prefix modules/ui-web run lint`, now `eslint . --max-warnings=0` so an unused `eslint-disable` fails too). **`scripts/**` + `packaging/**` JS**: ESLint `no-warning-comments` in the root `eslint.config.mjs` (step "Script lint", `npm run lint:scripts`), ratcheted by the root `eslint-suppressions.json` — 1 marker pinned. **`*.ps1`**: `scripts/ci/check-ps1-warning-comments.mjs` (same step, `npm run lint:ps1`), which ESLint cannot parse; ratcheted by `scripts/ci/ps1-warning-comments-suppressions.json` — 1 marker pinned. The first two parse, so unlike the gate's regex they cannot score the marker word inside a string literal or a URL. **One honest gap remains**: PMD is opt-in locally (`-PskipPmd=false`) and no workflow puts `pmdMain` in its task graph, so the Java `CommentContent` rule in `config/pmd/ruleset.xml` bites only when PMD is run. Wiring `pmdMain` in CI needs the 78 pre-existing violations of the OTHER rules in that ruleset (mostly `UnnecessaryFullyQualifiedName`) fixed first. (Closed by tempdoc 930 §22.2 follow-up 3.) |
