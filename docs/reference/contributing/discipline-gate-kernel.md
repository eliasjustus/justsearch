...
| `todo-fixme` | ESLint `no-warning-comments` (ui-web, wired in CI as `npm --prefix modules/ui-web run lint`) + the PMD `CommentContent` rule in `config/pmd/ruleset.xml` and `config/pmd/ruleset-cli-tools.xml` (Java). The Java half is live: `pmdMain` runs in `check`/`build` by default and as the `Static analysis (PMD)` step of CI's required `Build (no model blobs)` check (tempdoc 930 §22.2 follow-up 2 cleared the 130 pre-existing violations of the other rules — 109 of them `UnnecessaryFullyQualifiedName` — and flipped the `skipPmd` default). **One honest gap remains**: `scripts/**/*.mjs` and `*.ps1` are covered by neither linter, and the gate did scan them. |
