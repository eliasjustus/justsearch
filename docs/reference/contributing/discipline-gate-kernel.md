...
| `todo-fixme` | ESLint `no-warning-comments` (ui-web, wired in CI as `npm --prefix modules/ui-web run lint`) + the PMD `CommentContent` rule in `config/pmd/ruleset.xml` (Java). **Two honest gaps**, both narrower than they look because Java main sources and ui-web hold zero markers today: (1) `scripts/**/*.mjs` and `*.ps1` are covered by neither, and the gate did scan them; (2) PMD is opt-in locally (`-PskipPmd=false`) and no workflow puts `pmdMain` in its task graph, so the Java rule bites only when PMD is run. Wiring `pmdMain` in CI needs the 78 pre-existing violations of the OTHER rules in that ruleset (mostly `UnnecessaryFullyQualifiedName`) fixed first. |
