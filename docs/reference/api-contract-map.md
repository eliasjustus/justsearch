...
- `DELETE /api/chat/conversations/{sessionId}` also deletes the conversation's agent runs (tempdoc 859 slice C), which is what makes a run-backed row removable; the response carries `agentRunsDeleted`.
- `GET /api/chat/conversations/{sessionId}/history` does **not** perform that join: the unified transcript's one authority is `GET /api/thread/{id}`, which already merges the answer and action planes.
