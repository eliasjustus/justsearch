...
The response also carries `repositoryHealth`: `{available, scope: "local", capturedAt?, metrics?}`.
When available, `metrics` contains `gradleModuleCount`, `testFileCount`, and
`productionSourceLocByModule`. This is the latest schema-v2 `repository-health` row from the same
gitignored local history that records gate runs. It is retention-bounded inspection evidence, not a
durable or cross-machine trend; legacy gate-only history remains readable and simply yields
`available: false` for this projection.
