...
| `JUSTSEARCH_EMBED_LATE_CHUNKING_ENABLED` | `justsearch.embed.late_chunking_enabled` | Bool | Long-doc single-pass VECTOR embed path for chunked parent docs (tempdoc 691 Phase 2/4). **Default `true`** (default-on since 691 §Phase N: quality gates green — legal-clerc `vector` nDCG@10 0.0597→0.2967 at defaults / 0.3401 with a 6144MB embed arena; enron +7% vector, +1.3% hybrid; scifact neutral). When on, a chunked parent doc's `VECTOR` is embedded in one batch-1 pass at `JUSTSEARCH_EMBED_LATE_CHUNKING_CONTEXT_LENGTH` tokens instead of the base `JUSTSEARCH_EMBED_CONTEXT_LENGTH` window-mean. Measured cost: background enrichment is slower on long-doc corpora (enron 7.7→4.5 docs/s at the default 3072MB arena — long single passes cost more GPU than windowed batches; raising `JUSTSEARCH_EMBED_GPU_MEM_MB` to 6144 reduces OOM-fallback double-pays). Set `false` to restore the pre-691 windowed-mean behavior (throughput-first). The per-span `CHUNK_VECTOR` half was DROPPED per §Phase M's offline CLS measurement (nDCG@10 −0.2329) — chunk docs keep their separate per-chunk embed path regardless of this flag. |
