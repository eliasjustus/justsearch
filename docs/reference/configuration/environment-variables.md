...
| `JUSTSEARCH_EMBED_LATE_CHUNKING_ENABLED` | `justsearch.embed.late_chunking_enabled` | Bool | Enables the long-doc single-pass VECTOR embed path for chunked parent docs (tempdoc 691 Phase 2). Default `false`. When on, a chunked parent doc's `VECTOR` is embedded in one batch-1 pass at `JUSTSEARCH_EMBED_LATE_CHUNKING_CONTEXT_LENGTH` tokens instead of the base `JUSTSEARCH_EMBED_CONTEXT_LENGTH` window-mean — the F-030 dense-quality fix (`vector` nDCG@10 0.0597→0.3403 at 8192 tokens, legal-clerc-200). The earlier per-span `CHUNK_VECTOR` half (tempdoc 691 Phase 1) was DROPPED per §Phase M's offline CLS measurement (nDCG@10 −0.2329) — chunk docs keep their existing separate per-chunk embed path regardless of this flag. |
