...
| `JUSTSEARCH_SEARCH_MCP_FRAMING_CALIBRATED_ABSENCE_ENABLED` | `search.mcp_framing.calibrated_absence_enabled` | Bool | Tempdoc 789 Phase 2 (F3), **default false**. When enabled, zero-result and thin-result `justsearch_search` deliveries carry corpus coverage (indexed document count), what was searched, an explicit absence-is-not-evidence sentence, and a concrete next step. Probe substrate against over-trusted emptiness (the tool arm abstained 2x baseline at identical fabrication counts). |
| `JUSTSEARCH_SEARCH_MCP_FRAMING_THIN_RESULT_FLOOR_BYTES` | `search.mcp_framing.thin_result_floor_bytes` | Int | Tempdoc 789 Phase 2 (F3), default 400. Delivered-body byte floor below which a non-empty result set is still treated as "thin" and receives the calibrated-absence framing. Measures per-hit delivered text only (title + path + preview + matched terms), excluding response scaffolding and the framing lines themselves, so boilerplate cannot lift a substantively empty delivery over the floor. Only consulted when `calibrated_absence_enabled` is on. |
