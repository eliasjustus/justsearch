...
| `JUSTSEARCH_CONTEXT_SIZE` | `justsearch.context.size` | Int | LLM context window (`-c`). **Default: auto, derived** — override with `N` (tempdoc 883). Unset means the Head derives the window from the backend and contributes it at ordinal 150 (`auto_detected` / `hardware_probe`): 32768 with GPU layers, 8192 at `-ngl 0`, stepping down through 16384 → 8192 → 4096 if llama-server refuses a rung at launch. Any explicit value (here, `-D`, `settings.json`, or YAML) is a one-rung ladder: honoured or the launch fails loud, never silently reduced. `/props` `n_ctx` remains the authority for what the server actually has. |
