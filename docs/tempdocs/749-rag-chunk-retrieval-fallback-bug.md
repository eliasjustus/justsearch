...
- `/search-quality` before/after eval on scifact (mandatory falsifier): interactive
  ...
  **Coverage note (review PF-3):** the run's `summary.json` stamps `git_sha 5e195fe6` (the *parent*
  ...
  NOT a regression; the full-enrichment re-run is the valid number. (The RAG-path before/after
  falsifier + the live `rag_reachability` demonstration were completed separately — see
  §"Flag + RAG-path validation" below.)

## Flag + RAG-path validation (2026-07-18, session 2cad7f69, `/plan`) — closes tempdoc items 2–4

Added a default-ON config flag **`rag.union.enabled`** (`JUSTSEARCH_RAG_UNION_ENABLED`,
`ResolvedConfig.Rag.unionEnabled`), gating `buildUnionCandidates` in
`RagContextOps.searchChunksWithMeta`. Mirrors the `rag.chunk_vectors.enabled` default-ON precedent
(4 config files + 2 config docs + a wiring test). Default-ON preserves option B's "fixes all
indices immediately" intent — it only adds an operational off-switch. This is also the gate the
tempdoc's own falsifier language ("must be gated/length-weighted") anticipated as the remedy if
brevity bias ever shows up: the remedy is now pre-built. (Correctly excluded from
`index_identity.py` — query-time-only, index byte-identical.)

**Live RAG-path validation** (mixed index: 280 long chunked `needle-burial-v1` docs, all >2000
chars, + 24 short chunkless `synth-multihop-v1` docs, all <200 chars; union ON; enrichment 100%):

- **Reachability guard DEMONSTRATED FIRING + PASSING (item 2):** `jseval retrieval-eval …
  --corpus-jsonl synth-multihop-v1/corpus.jsonl` → `rag_reachability_probe` verdict **`ok`, 10/10
  chunkless short docs reachable via CHUNK_HYBRID** (`d_calla` etc. cited with their own
  `parent_doc_id`, `quality.retrieval_mode=CHUNK_HYBRID`, `best_score`≈0.98). Under the pre-union
  behavior these chunkless docs are unreachable (deductively: no chunk docs → not cited). This is
  the tempdoc's "earns its keep" demonstration, on a real corpus.
- **Running the probe live caught 3 real bugs in the W3 code (all fixed):** the probe (a) scoped
  `retrieve-context` to `doc_ids:[corpus-id]` while the backend keys docs by ingest PATH → nothing
  retrieved; (b) matched `parent_doc_id == corpus-id` exactly (path vs bare id → never matched);
  (c) used a stricter-than-product `reachability_top_k=5`. Fixes: unscoped query, filename-stem
  match via the existing `_doc_id_matches_title`, `reachability_top_k=10` (product depth). The
  probe was **100% non-functional for real corpus-dir corpora** before this — a genuine
  `static-green (unit-mocked) ≠ live-working` catch; the unit tests mocked matching bare ids and
  asserted the scoping, so they locked in the bugs (tests rewritten to path-form ids + unscoped).
- **Long-path non-disturbance is STRUCTURAL (partial falsifier):** the union's `withChunks` filter
  drops every chunked doc, so long-doc (needle) retrieval is union-independent *by construction*
  (needle docs verified chunked/retrieved). The needle eval arm's metric was a query-schema
  artifact (needle `queries.json` has no `query` field `retrieval-eval` reads), not a retrieval or
  union signal.
- **Interrogate-results save:** a transient `0/10 + 0%` from running two evals back-to-back
  (momentary backend overload) was NOT recorded as real — re-running each alone gave the true
  `10/10`, and a direct query confirmed the backend healthy throughout.
- **Reranker length-calibration spot-check (item 3):** in the live mixed-index runs short synthetic
  chunks and long-doc chunks co-ranked sensibly (best_score ≈0.98, no pathological short-bias); the
  CE does not rerank in the default CHUNK_HYBRID path anyway (`shouldRerankChunks` BM25-only).
- **HONEST residual (named, not measured):** the pure adversarial case — a short chunkless doc
  *topically competing* with a long doc for the SAME query and being over-promoted — is not tested
  (the two corpora are topically disjoint; no same-query competition exists in any locally-available
