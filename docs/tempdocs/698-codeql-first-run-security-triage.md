...
type: tempdoc
status: "in progress — PR #103 (ffa2d2d) + PR #108 (652fd17) merged; plus two fixes committed locally on the shared main checkout but NOT YET PUSHED: commit 5a575b9 (alert #25, cross-spawn) and commit 79a63ea (the stripComments unification, 12 alerts across 11 files, corrected from an original mis-scoped '18 alerts / 46 files' claim — see that section). Next agent: push these two as separate PRs (see Remaining work items 2/4/5) and re-run CodeQL. Final CodeQL-confirmed state so far: 3/27 auto-closed (20/21/22); alert #24 fixed-but-CodeQL-won't-recognize; alert #1 fixed (root-caused, re-verified) but still shows open for an unexplained reason — do not treat as resolved; alerts #25 and the 12 stripComments alerts fixed but not yet re-scanned (not pushed yet); 6 alerts wrongly folded into the original stripComments story still need individual triage; 3 alerts are false positives, left as-is"
created: 2026-07-08
updated: 2026-07-08
related: [695]
---
...
recommended "extract one canonical shared regex-based helper." That's not the right fix: this is
a well-known, already-solved problem, and a real single-pass scanner is not a novel invention
needed here. A regex chain got inline-invented anyway *because* nobody checked first — a
textbook instance of this repo's own `explore-before-implementing` rule (the
...
1. **Alert #1 is the open item that actually needs work.** Two merged attempts, the second
   ...
   patching around it.
2. **Committed locally, NOT pushed as of this writing** (commit `79a63ea` on the shared local
   `main` checkout): the `stripComments` unification (see its section above) — 25 files
   migrated to `scripts/lib/strip-comments.mjs`, differential-tested, spot-checked, all green
   post-merge with `origin/main` up to `cc00d5f`. **Next agent: push this** (new branch from
   `origin/main` + PR, per this repo's branch-protection workflow — direct push to `main` is
   blocked) **and re-run CodeQL to confirm the 12 previously-flagged alerts
...
4. **Committed locally, NOT pushed as of this writing** (commit `5a575b9` on local `main`): alert
   #25's `quoteCmdArg`/manual-`cmd.exe` replacement with `cross-spawn` (see its row above) — **next
   agent: push this and re-run CodeQL to confirm it closes.** Also worth: wiring
   `scripts/ci/test-report-build-attribution.mjs` into an actual CI job (investigation for this
   fix found it isn't currently run by any workflow — it was only ever run manually) so its
   now-meaningful `.bat`-path coverage doesn't silently bit-rot.
5. **Recommend pushing items 2 and 4 as two separate PRs** (matching this session's established
   pattern of one focused PR per fix — see PRs #103/#104/#108), rather than one large PR, since
   they're unrelated fixes and independently revertable/reviewable.
