...
3. Lane F derisk — **no-go for now** (owner, 2026-09-05): a process-boundary rewrite (917: 1,806-line Head client, 48 RPCs, spawner, 20 chaos tests, dev-runner) waits for D fully merged, the C1 decision closed, and a brief refreshed with 917's corrections. **2026-09-06: all three waits cleared** — D merged (§D), C1 decided (item 1), the brief refreshed as `917-evidence/lane-F-brief-v2.md` and 917's four derisk items run (917 §Derisk results: Head heap baseline measured, `energy_reduced` re-homing added to PR 1, three 885 constructs 917 had missed, ArchUnit composition redesigned — the worker-isolation rules were never the blocker). Remaining gate: the owner's go-ahead.
