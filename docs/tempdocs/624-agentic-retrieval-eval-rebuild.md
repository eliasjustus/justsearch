...
# §M.8 item 6 CLOSED — claim text signed off; published (2026-07-03, twenty-third pass)

Founder sign-off recorded (session directive, 2026-07-03, ordering publication prep on the drafted
claim text verbatim). Actions: RESEARCH.md updated from the stale pre-certification figure to the
certified null (claim text verbatim, marker-wrapped); README agent-utility language aligned;
Design 2's pre-registered trigger has fired (RESEARCH.md now on `main` citing this finding) and the
purpose-built projector `scripts/ci/check-agent-utility-claims.mjs` is built and wired into
docs-lint, sourcing the committed `utility-comparison` records — the claim can no longer drift
silently. §M.8 is now fully satisfied; this effort's certification arc is complete. Round two
(battlefield-v2: exploration-infeasible scale, real formats, cross-lingual member, coverage-shaped
pre-registered claims, tier sensitivity) is named in RESEARCH.md as designed-not-yet-resourced.
