...
- ~~`release.v1.json` anchored to a dead commit + retired corpora~~ — **RESOLVED by tempdoc 666** (merged to `main`
  2026-07-01): the release is re-anchored to a live commit + the current CLERC/MIRACL/enron/scifact cohort.
- ~~`jseval perf-gate`'s default `--baselines` path resolves one level too deep~~ — **RESOLVED on `main`**:
  `gates.py` now uses `parents[2]` + `tests/test_gates_default_paths.py` guards all five default baseline paths.
...
- **Recompose `release.v1.json` once 647's metrics are on `main`** → activates the new `retrieval_p50_ms` +
  footprint-component floors. This is now a *routine* recompose (666 already re-anchored the cohort — no
  corpus-integrity decision remains); just a fresh cohort run + `jseval release`.
