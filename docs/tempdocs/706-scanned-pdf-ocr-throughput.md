...
type: tempdocs
status: "IMPLEMENTED + verified + review-hardened (2026-07-10, same day as design): S1 engine+teardown (8e882d1), S2 config unification (159653d), plus a post-implementation review round that found and fixed one real defect the first review missed (dropped per-rendered-page size guard — restored pre-render) and hardened the evidence record via an independent refute-first claims audit (before-timing re-measured durably: 115,055 ms → 16,774 ms ≈ 6.9×). Full unit suite + module suites green; two independent reviews (code SHIP verdict; claims audit). Register F-030 records the extraction-content comparability boundary. Open decision item: the now-actually-binding 50-page cap (§review round). MERGED to main 2026-07-10 as PR #124 (squash 40138f7) together with 686/705/677 docs; the full-corpus 686 re-run happened pre-merge (extraction ~5x corpus-level, see 686 §run 2); page cap raised 50->200 in the same PR. One post-merge hardening rider: PR #128 (temp-dir sweep UncheckedIOException escape, CI flake). Spun out of 705's founder-directed sidegoal per 686's own out-of-scope rule ('fixing whatever the measurement finds' is a new tempdoc). Owns OCR execution performance + the extraction timebox/orphan liveness fix; does NOT own routing (607), reason codes (671, shipped), or the extraction-tax verdict (705)."
created: 2026-07-10
