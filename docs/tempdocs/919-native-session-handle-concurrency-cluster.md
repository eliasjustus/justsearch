...
type: tempdocs
status: PARTIALLY SUPERSEDED (2026-09-05) — the fail-closed "never fall to CPU" GPU policy designed
  in §4.5 is NOT to be implemented as specified: CPU fallback is shipped and relied on today
  (`NativeSessionHandle.getCpuSession()` call sites at `NativeSessionHandle.java:213,218,248`;
  `cpu_fallback.triggered` telemetry, `docs/explanation/08-observability.md:581`; ADR-0004's
  2026-06-17 update, `docs/decisions/0004-single-tenant-gpu-policy.md:96`), so ruling it out here
  would contradict shipped, depended-upon behavior rather than replace it. Process separation for
  `llama-server.exe` (the Worker-process-exit-as-recovery-unit shape) stands and should still be
  pursued. Decision source: tempdoc 930 §14.1 and §18.1 row 10; see "Status update 2026-09-05" below
  and §Status. Originally: DESIGNED (2026-09-03) — theorized + designed; open questions answered
  (§7); not started. Takes over 900 items 2-3 and 898 item 2 (see §0). Implement in the order of §9;
  do not re-litigate the decisions in §4 without writing why in §Status.
created: 2026-09-03
updated: 2026-09-03
...
The fail-closed GPU policy designed in §4.5 ("mark, exit, supervised restart, encoders-off on
repeat — never fall to CPU") is **not to be implemented as specified**: CPU fallback is shipped and
relied on in this codebase today, not merely a residual to clean up later — see the
`getCpuSession()` call sites in `NativeSessionHandle.java` (~lines 213, 218, 248), the
`cpu_fallback.triggered` telemetry event (`docs/explanation/08-observability.md:581`), and ADR-0004's
2026-06-17 update narrowing full-eviction behavior around CPU fallback
(`docs/decisions/0004-single-tenant-gpu-policy.md:96`). Ruling out CPU fallback here would silently
regress that shipped, depended-upon behavior instead of superseding a stale design. The parts of this
tempdoc that keep process separation for `llama-server.exe` as the recovery unit (§4.4 close
semantics, the Worker-process-exit-and-supervised-restart shape) are unaffected and should still be
pursued. This decision comes from tempdoc 930 §14.1 and §18.1 row 10; §4.5's device-lost design
below is left intact as historical record, not as an implementation instruction.
