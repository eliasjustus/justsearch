...
type: tempdocs
status: "PHASE 1 IMPLEMENTED + INDEPENDENT REVIEW APPLIED (2026-09-03) — five parity keys replaced by index_fingerprint + boosts_fp; the Head no longer disables the parity guard; production default is BLUE_GREEN_MIGRATE with a bounded repeat-rebuild brake. Phase 2 (document identity) is DESIGNED (§P2, decisions in §P2.E). Phase 3 (reindex bundle) is DESIGNED (§P3) — the pass found a codec blocker (O15) and a trace defect (O16) the brief did not state. Neither phase is implemented."
created: 2026-09-03
updated: 2026-09-03
...
### Phase 3 — the reindex bundle, one migration for users (DESIGNED — see §P3.B/§P3.C; not implemented)

Pre-implementation pass and tightened design: **§P3** (below §P2.E). The brief's C1 turned out to
carry an unstated blocker (the codec records no per-segment format, §P3.B P5) and C3/C4's consumer
lists were both incomplete (P23, P30); C5's verification is impossible until a trace defect is
fixed (P37). Each item below now names its design subsection.

- [ ] C1. (design §P3.C(1)) Quantized vectors by default, with jseval nDCG@10 / recall@50 evidence (delta ≤ 1%
      absolute), index size and RSS before/after; binary-quantized HNSW on `chunk_vector` as a
      report-only experiment.
- [ ] C2. (design §P3.C(2)) Pin vector similarity: add `vector.similarity: dot_product` to both catalog copies,
...
- [ ] C3. (design §P3.C(3)) Stop storing `chunk_content` (`stored:false`, still indexed); slice the parent `content`
      by `chunk_start_char`/`chunk_end_char`; measure the per-hit stored-field cost.
- [ ] C4. (design §P3.C(4)) Delete the `entity_*_text` fields and the entity text-boost path; keep facets on
      `entity_*_raw`; tell lane B for ADR-0007's amendment.
- [ ] C5. (design §P3.C(5)) Replace the English stop-word list with a document-frequency signal; verify comparable
      per-language skip rates on the multilingual eval sets.
- [ ] C6. (design §P3.C(6)) Chunk size: take lane E's number; change nothing about chunking except that its
...
- **Lane B — ADR-0007 amendment, ready to finalise (Phase 3 §P3.C(4)).** Lane D's design deletes
  `entity_persons_text` / `entity_organizations_text` / `entity_locations_text` from both catalog
  copies, the `justsearch.search.entity_boost` key, and the whole `TextQueryOps` entity path. The
  `adr-0007-entity-boost-retired-or-off` probe (`governance/adr-probes.v1.json:100-122`) then passes
  by its **second** alternative (`grep-absent` in `SSOT/catalogs/fields.v1.json`) instead of its
  first — verified predicate, §P3.B P32. The amendment text can say: entity FACETS on
  `entity_*_raw` are the shipped answer; the text-boost fields are retired (F-010); re-introducing
  them is a reindex. The probe's `note` cites a stale `ResolvedConfigBuilder.java:1336` (the key is
  at `:1446`) — lane D fixes that comment as a ride-along.
- **Lane E — two numbers lane D needs, and one it does not.** Lane D does NOT need recalibrated
  threshold values: §P3.C(2) shows both 702 constants map by exact arithmetic (`0.294 → 0.40`,
  `1/3 → 0.5`) because they were authored in cosine-score space to begin with (P13). What lane D
  does need from lane E is (a) whether `cc_alpha` / the CC weights move once the dense leg's score
  *spacing* changes (P14 — min-max normalisation is not invariant to the non-affine warp), and
  (b) current `primary_docs_s` / `enrich_docs_s` so the Phase-3 evidence campaign can be sized
  (Q-P3-7's 12-18 machine-hour estimate is arm-count × register figures, not measured).
...
4. **O4 — CLOSED (Phase 3 §P3.B P23/P25).** The list was re-derived from scratch: **15 sites in 9
   files**, four of them index-time backfills the brief does not mention at all. `HighlightingOps`
   still has zero `CHUNK_CONTENT` references (D3 stands) but *does* receive chunk text through
   `SearchResponseBuilder.java:559-572`'s `spanFields` substitution (P25).
...
15. **O15 — OPEN, and it blocks C1. Found by the Phase-3 pass (§P3.B P5).** `JustSearchCodec`
    overrides `knnVectorsFormat()` to return a raw format instead of the base codec's
    `PerFieldKnnVectorsFormat`, so **nothing records which vectors format wrote a segment**; on read
    the codec is SPI-resolved by name through its no-arg constructor, which is pinned to Float32.
    An index written with `index.vector.quantization.enabled=true` is therefore unreadable after a
    restart. Latent today (the key defaults false and the only exercise is a same-JVM benchmark),
    and it retro-explains the 2026-01-21 demotion comment. Fix designed in §P3.C(1)(a)3 (a second
    SPI-registered codec name); it is a prerequisite for C1, not an optional cleanup.
16. **O16 — OPEN, and it blocks C5's verification. Found by the Phase-3 pass (§P3.B P37).**
    `SearchTraceProjector.legsOfMultiLeg:157-176` derives the dense-retrieval stage status purely
    from the planned `LegSet`, but `HybridSearchOps.shouldSkipVectorSearch` suppresses the dense leg
    *below* the plan (`HybridSearchOps.java:361`, `ChunkSearchOps.java:557,619,729`). Every skipped
    query is reported as `dense-retrieval: executed` with no reason — on the trace, in the FE
    explain, in the MCP degradation summary, and in eval. Same defect class as register F-012.
    Fixed as a side effect of §P3.C(5) option 2 (move the decision into `SearchPlanner`); if that
    grant is refused (Q-P3-3), this routes to lane E as a search-observability defect and the
    brief's multilingual skip-rate verification cannot be performed.
17. **O17 — ROUTED, `/ssot-catalog` skill owner.** The dual-copy rule covers
    `SSOT/catalogs/fields.v1.json` and the adapters-lucene resource, but there is a **third**
    hand-maintained copy of the field list in Java: `FieldCatalogDef.java:109-160`
    (`forChunkTesting`), which mirrors `stored`, roles and vector dimension and has no similarity
    slot (§P3.B P27). Every Phase-3 `fields[]` change must touch it; the skill should say so.
18. **O18 — ROUTED, ADR-0043 / gate owner.** `check-language-agnostic-analysis.mjs` does not and
    cannot see the 82-word English list in `HybridSearchOps.java:81-91`: its four checks are
    analyzers, `content_<lang>` catalog fields, per-language files *under the catalogs dir*, and
    `content_<lang>` query-path literals (§P3.B P38). ADR-0043's own backstop missed the artifact
    ADR-0043 forbids. §P3.C(5) adds a fifth check in the same PR that deletes the list.
