...
| **L16** → **905** | Release unblock | 9.1, 9.2, 9.4, 9.6 | OWNER(2 confirmations) + OPUS(now) | vendor is **eSigner (SSL.com), already documented, validated and in use** (`cut-a-release.md:326-341`; signed round-17 candidates in 823 §6; trial window ends ~2026-09-09) — the register's 9.1 "unsigned pending cert/vendor" was stale on that point and the first 905 draft's Azure proposal was wrong, both corrected; GA tag `v0.2.0` only from a signed, 617-qualified candidate; winget then Scoop, no Chocolatey; `CHANGELOG.md` becomes the release-body source with a gate. Items 3-5 startable now; items 1-2 wait on the owner confirming the production eSigner secrets + post-trial plan and the winget publisher identity |
...
founder can perform: confirm the existing `justsearch-perf` runner is online and pick the
schedule hours (904 §O), confirm the production eSigner secrets and the post-trial plan plus
the winget publisher identity (905 §O), push/PR the branches, and start opus threads.
