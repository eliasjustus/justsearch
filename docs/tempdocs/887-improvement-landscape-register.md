...
| **L2** → **904** | CI enforcement closure, tier 1 (self-hosted / GPU) | 4.6, 7.4, 4.4, 7.7 pytest, 7.8 | OWNER(15 min) → OPUS | decided: the owner's dev machine as a self-hosted scheduled/dispatch runner (numbers stay comparable with the register's baselines); owner installs the runner (904 §O), then opus wires perf-gate, PIT ratchet, soak, full pytest, dispatch-only upgrade qualification |
...
| **L16** → **905** | Release unblock | 9.1, 9.2, 9.4, 9.6 | OWNER(2 actions) + OPUS(now) | decided: Azure Trusted Signing via the existing `command` mode (eSigner as validated fallback); GA tag `v0.2.0` only from a signed, 617-qualified candidate; winget then Scoop, no Chocolatey; `CHANGELOG.md` becomes the release-body source with a gate. Items 3-5 (winget/Scoop generation, CHANGELOG gate, model-swap runbook) startable now; 1-2 wait on the owner creating the signing account and the winget publisher identity |
...
founder can perform: install the self-hosted runner (904 §O), create the signing account and
winget publisher identity (905 §O), push/PR the branches, and start opus threads.
