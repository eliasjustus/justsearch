...
type: tempdocs
status: "REGISTER COMPLETE + 13 LANES CHARTERED (2026-09-02) — 75 items existence-checked by 10 pinned opus workers; decision-age audit (§S) applied; handoff tempdocs 888-900 written for the opus lanes (L1,L3,L5,L6,L7,L8,L11,L12,L13,L14,L15,L17,L19); fable designs settled: L4 → 901, L9 → 902, 1.1 re-read → 903, L10 → 907, failure-UX pass → 906; L2 → 904 and L16 → 905 chartered; L18 decided per item; all 21 tempdocs (887-907) published together 2026-09-02"
created: 2026-09-02
updated: 2026-09-02
...
|---|---|---|---|---|---|
| 1.1 | Non-NVIDIA inference (DirectML/Vulkan/ROCm) | RE-EXAMINED → **DECIDED (b)** | OPUS → **903 §6** | L18 | 903 (research complete 2026-09-02, measured on the dev card): llama.cpp Vulkan at parity with CUDA on the packaged model at the pinned tag; ORT WebGPU plugin EP works from Java from ORT 1.24.4; **decision: option (b) — ship the Vulkan llama-server variant for chat on AMD/Intel, encoders stay CPU there; option (c) (WebGPU encoders) rides the next ORT bump; (a) rejected**. Chartered as 903 §6 (one opus lane, M). First AMD/Intel same-box measurement per 903 §2.3 is the reassess trigger |
| 1.2 | CPU-only degraded tier | PARTIAL (corrected by 903) | → 903 §6 | L18 | **search only, no chat**: encoders run FP32 on CPU, but `InstallPlanner.java:210-229` skips every GGUF unless the profile `includesGguf()` (only `GPU_FULL`, `DownloadProfile.java:19`) — skip reason "CPU chat is not supported in this build"; the 8k CPU rung in ADR-0047 is never reached by an install path. The group-1 audit's "chat runs degraded on no-GPU" was wrong. 903 §6 closes it for Vulkan-capable GPUs; a true CPU-chat install stays a non-goal (minutes-per-ask prefill) and `05-ai-architecture.md:83-85` (§Z-2) plus `840:107` must say "search only without a supported GPU" |
...
| 2.9 | Non-Windows portability | PARTIAL | DEFERRED (decided) | L18 | 761 holds a complete `file:line` inventory (Linux L, WSL M); its hardest blocker (no Linux-CUDA llama-server prebuilt) is dissolved by 903's Vulkan decision (ggml-org ships ubuntu Vulkan assets), so the cost class drops from L toward M — **still not chartered**: re-estimate after 903 §6 ships the Vulkan variant on Windows, then decide; `docs/comparison.md:20` stays "Windows-only" until then |
...
| id | item | verdict | bucket | lane | gap |
|---|---|---|---|---|---|
| 9.1 | Code signing | PARTIAL (corrected) | OWNER-confirm | L16 | three-mode signing + verify + sign-once mirrors wired; **vendor is eSigner (SSL.com), validated and used for signed round-17 candidates (823 §6), trial window ends ~2026-09-09** — the group-9 audit's "pending cert/vendor purchase" understated this; what is open is production-secret confirmation, `REQUIRE_SIGNING` on the release path, and the post-trial plan |
...
| 9.7 | Update integrity | SHIPPED (corrected) | DROP | — | Ed25519 descriptor + minisign + SHA256SUMS round-trip shipped; rollback is a locked non-goal (617 D5); v0.2.0 is published (2026-08-13) so "qualification pending on GA" is stale — 905 item 2 closes 617's status |
...
Each lane becomes a tempdoc only when picked. Bucket = who acts first. **Chartered 2026-09-02:** the 13 OPUS lanes have handoff tempdocs 888-900 (number after the arrow); L4 → 901 and L9 → 902 are settled designs; the 1.1 re-read is 903 (research complete, option (b) decided); L2 → 904 and L16 → 905 are chartered with their owner actions named; L10 → 907 is a settled design whose gauge-wiring chunk keeps the L10 trigger as its start condition; the failure-UX pass named in L18's coverage note → 906; L18 is decided per item.
...
| **L2** → **904** | CI enforcement closure, tier 1 (self-hosted / GPU) | 4.6, 7.4, 4.4, 7.7 pytest, 7.8 | OWNER(5 min) → OPUS | decided: use the **existing** `justsearch-perf` self-hosted runner (`docs-lint.yml:20`) for scheduled + dispatch jobs from `main`; threat evaluated in 904 §Decision — fork-PR RCE is already forbidden by the hard invariant in `check-workflow-triggers.mjs` (747 P-D), residuals are owner-merged code, tag-pinned Actions (SHA-pin), secret scoping, workstation load (owner-chosen hours); 904 item 0 amends ADR-0044; owner only confirms the runner and picks hours |
...
| **L10** → **907** | User indexing policy | 6.4 | FABLE → OPUS (design settled) ⇢882-C | originally deferred (designing a user pause/schedule lever on top of an unmeasured automatic duty cycle would re-open lane C's decision); founder directed the design be written 2026-09-02 → `907-user-indexing-policy` (settings shape: pause / pause-on-battery / schedule window, projected onto 885's gauge as an input, never a second pacing authority). The original trigger — 885 §Status "live arms recorded" + 813 finding 3 settled — survives verbatim as the **start condition of the gauge-wiring chunk**; store, settings surface and operation chunks may start now |
...
| **L16** → **905** | Release unblock | 9.1, 9.2, 9.4, 9.6 | OPUS(now) | verified read-only 2026-09-02: eSigner (SSL.com) secrets set 2026-08-12, signed mirrors run 2026-08-12, **v0.2.0 published 2026-08-13 as latest** — the group-9 rows 9.1/9.7 ("unsigned pending vendor", "qualification pending") and the first 905 draft (Azure, GA pending) were stale/wrong, all corrected in 905; founder budget decision: pay the eSigner tier post-trial, never exceed the monthly allocation (sign-once mirrors + a signing counter enforce it); winget submission for the existing v0.2.0 is startable now, then Scoop, no Chocolatey; `CHANGELOG.md` becomes the release-body source with a gate; 617's "pending" status to be closed. Owner only opens the winget-pkgs PR |
...
| **L18** | Product bets — DECIDED per item (2026-09-02, founder delegated) | 1.1, 1.4, 2.9, 5.4, 5.7, 5.8, 6.2, 6.6, 9.3, 10.3 | see per item | **1.1** non-NVIDIA re-read → **903 complete; decided option (b)** (Vulkan chat for AMD/Intel, encoders stay CPU; opus chunk 903 §6); it also corrected 1.2 (no chat without a CUDA GPU today) and re-costs 2.9. **1.4 ARM64**: NO for now — no test hardware, no ORT/llama ARM64 supply chain, revisit when 903 or a user report evidences demand. **2.9 Linux/WSL**: DEFER until 903 resolves the llama.cpp backend question (761's hardest blocker is Linux-CUDA; a Vulkan answer changes the cost class); no charter. **5.4 collection intelligence**: NO — no evidence of demand, and folder summaries would be model-authored text re-entering retrieval (901's origin axis) before that axis exists. **5.7 multimodal query**: NO — a second embedding space and model pack; out of the 0.2.x horizon. **5.8 LambdaMART enablement**: handed to lane E (ranking) with the evidence 895/897 produce; a default flip needs an offline eval on captured dispositions first (778 §B.3 gate stands). **6.2 OS integration**: Explorer context menu and file associations stay NO (shell-extension DLL cost; 191's "marginal" holds for associations); file preview surface re-examined when 890 lands (the palette is the natural host). **6.6 UI localization**: NO until a non-English user base is evidenced; the German signal (748) is retrieval-side. **9.3 integration plugins**: YES for exactly one — a PowerToys Run plugin — chartered only after 890 ships the hotkey/palette (it reuses the same loopback search call); Raycast/Obsidian/VS Code/browser stay NO. **10.3 product KPI**: NO KPI under the no-telemetry invariant; the register's proxies (eval lanes, release qualification, GitHub issue intake) are the observation; a local-only "your usage" panel is a feature, not a metric, and is not chartered. **Coverage note (2026-09-02):** two lenses from the original theorization were never itemised and are now folded into charters — the canonical-doc claim sweep (893 item 7) and `forced-colors` + an owner-run assistive-technology checklist (890 item 5); a third, "failure UX as one coherent surface" (are `readinessNotice`, degradation codes, ingestion ledger reasons, and API error classes one vocabulary to the user?), is chartered as `906-failure-ux-coherent-surface` (2026-09-02, founder-directed ahead of 889/896): the vocabulary inventory and the one presentation authority start now; the chunk that folds in the 889/896 reason-code families is BLOCKED-SOFT on those lanes landing |
...
**Founder-delegated decisions (2026-09-02).** The founder delegated every owner call in this
register to the orchestrating session; decisions are recorded inline above (L2, L10, L16, L18)
and in 901 §K (K1-K4) and 902 §K (K1-K3), all with grounds. Remaining *actions* that only the
founder can perform (after the 2026-09-02 read-only verification closed the rest): veto the
proposed schedule in 904 §Schedule, open the `winget-pkgs` PR when 905 item 3 drafts it, and
start opus threads. Branches are pushed as draft PRs by the orchestrating session.

**Start order** (decided): 888 → 901 C1 → 889 → 890 → 892 → 905 items 3-5 → 902 I-1 → 893 →
894 → 895 → 900 → 897 → 898 → 899 → 891 → 896 (after 885 arms) → 904 (after §O) → 905 items 1-2
(after §O). Added 2026-09-02: 906's inventory + authority chunks and 907's store/settings/operation
chunks are self-contained and slot in after 892; their gated chunks wait on 889/896 and on the L10
start condition respectively. Rationale: 888 first because every later lane's acceptance criteria assume CI bites;
901 C1 second because it removes the live `.env` exposure on its own; 889/890 next because they
are user-visible and self-contained.
