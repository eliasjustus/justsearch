...
Each lane becomes a tempdoc only when picked. Bucket = who acts first. **Chartered 2026-09-02:** the 13 OPUS lanes have handoff tempdocs 888-900 (number after the arrow); L4 → 901 and L9 → 902 are settled designs; the 1.1 re-read is 903 (in progress, separate session); L2 → 904 and L16 → 905 are chartered with their owner actions named; L10 is deferred by decision; L18 is decided per item.
...
| **L10** | User indexing policy | 6.4 | DEFERRED (decided) ⇢882-C | decided: not chartered until 885's live arms and 813's cancel-honesty decision are recorded — designing a user pause/schedule lever on top of an unmeasured automatic duty cycle would re-open lane C's decision; trigger = 885 §Status "live arms recorded" + 813 finding 3 settled; then a short fable pass (settings shape: pause / pause-on-battery / schedule window, projected onto 885's gauge as an input, never a second pacing authority) |
...
(after §O). Rationale: 888 first because every later lane's acceptance criteria assume CI bites;
