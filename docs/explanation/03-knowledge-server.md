...
*   **Recycling**: a child is killed and respawned on a missed deadline, on a crash, and after `justsearch.extraction.sandbox.max_requests` requests (default 500 — the leak guard). Each event increments `extraction.sandbox_restart_total{reason}` (`timeout` | `crash` | `oom` | `request_budget` | `protocol` | `interrupted`); spawns increment `extraction.sandbox_spawn_total`.
