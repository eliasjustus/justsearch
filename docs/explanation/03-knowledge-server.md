...
*   **Startup probe**: because spawning is lazy, a broken child command would otherwise be invisible until the first file and would then fail every file. At wiring time the Worker spawns one child and runs a trivial extraction through it (bounded, 20s). On failure it logs a WARN naming the reason, records `reason=probe_failed`, and falls back to in-process extraction for the session — degraded, but every document still indexes.
