...
* **Runtime propagation:** `SettingsController.handleUpdateSettingsV2()` propagates the remaining promoted keys to system properties immediately when settings are updated (via `maybeApplyGpuLayersSysProp()` and siblings), then rebuilds the `ConfigStore`. Note that **inference restart is required** for GPU layer changes to take effect on the running `llama-server` process.
