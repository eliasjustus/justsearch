...
In the current app:
* `HeadlessApp` applies a few remaining `UiSettings` into canonical system properties (`justsearch.index.base_path`, `justsearch.llm.model_path`, `llama.lib.path`) using **"set only if blank"** semantics, the first two with a parallel `*.source=ui_settings` marker. This lets UI settings drive defaults without overriding explicit `-D...` / env configuration. Blank paths in settings are treated as "unset".
* **These promotions are legacy and are being retired** (tempdoc 883 decision 4). They predate the ordinal chain and are a precedence lie while they last: a GUI value written as a system property resolves at ordinal 500, so `/api/debug/effective-config` reports it as `jvm_arg`. `settings.json` already contributes at its own ordinal (300) via `ConfigStoreRebuilder.contributeUiSettings`, which is the mechanism that does not lie.
...
* **`justsearch.server.exe.source` still exists**, but only as the ownership token of a *runtime GPU-variant switch* (`RuntimeActivationService`, `HeadlessApp.maybeAutoSelectCuda12Variant`, `AiInstallService.applyCudaServerExe`), which writes a path discovered on disk. That is not a settings promotion; a grep for it is expected to hit.
...
* **Attribution nuance:** a small amount of legacy UI setting plumbing still sets sysprops without a `*.source` marker (e.g., `llama.lib.path`). When debugging “which config won?”, treat unmarked sysprops as **ownership-ambiguous** unless a marker exists.
