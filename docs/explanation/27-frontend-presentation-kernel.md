...
| indexing / enrichment PROGRESS numbers (phase, queue counts, coverage percent, estimate) | `state/indexingProgress.ts` `selectIndexingProgress` — one projection over the single `/api/status` poll snapshot the store already holds (no second transport), including which enrichment stages are APPLICABLE, so every number a progress surface shows comes from one snapshot and one derivation; surfaces may differ by SCOPE, never by derivation | the status-bar queue chip (`StatusDeck`), the Tasks panel (`TaskList`), Health's Queue card and "Now" strip, `IndexingOverlay`, and — for the per-folder second tier ("keyword-ready · enriching N%" → "fully searchable") — `state/folderStatus.ts`, which stays the ONE per-folder verdict site (`check-folder-status-derivation`) and consumes the applicability its wire row cannot carry |
