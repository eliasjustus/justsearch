...
| `confirm` | a floor on the agent gate — the declaration may tighten the lattice's verdict, never loosen it. It does **not** decide *whether* a confirmation runs; the lattice does (see `GateBehavior`'s javadoc). No `ExecutorTag.AGENT` operation may declare `Typed`: the agent authorization ceremony renders the operation id as the phrase and cannot carry a declared one. A registry test enforces that boundary rather than leaving the phrase to be silently replaced at runtime. |
