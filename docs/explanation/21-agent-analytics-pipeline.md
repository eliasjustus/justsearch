...
| `skill-delivery.mjs` | Inventories the checked-in Claude skill sources and generated Codex projections, then audits Codex rollout tool exchanges that read a project `SKILL.md`. Exact containment of the complete current file proves `proven_full_current`; explicit tool-result truncation, intentional partial reads, ambiguous batching, missing results, and unproven historical/current mismatches remain separate classifications. The reader does not infer that a capped batched result omitted a particular section, reconstruct cumulative coverage from multiple partial reads, or claim that delivered text was attended to or obeyed. Human and `--json` output contain aggregates only; raw prompts, commands, and outputs stay in memory. CLI: `--since`, `--until`, `--repo-root`, `--codex-home`, `--project-pattern`, `--json`. |
