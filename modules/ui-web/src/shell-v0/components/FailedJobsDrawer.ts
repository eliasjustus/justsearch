...
// off untyped JSON, so a backend that renames or drops it fails the parse boundary loudly instead
// of silently rendering every row as a plain parse failure.
...
export class FailedJobsDrawer extends JfElement {
  ...
  private async refresh(): Promise<void> {
    ...
    try {
      ...
      // The isArray check covers the PROD posture only: parseWireContract throws in dev but
      // degrades to the raw body in prod, where `jobs` is whatever the drifted backend sent.
      this.rows = Array.isArray(parsed.jobs) ? parsed.jobs : [];
