 ...
 * commits, cancels and reverts by the donor's rule.
 *
...
/** The window measured as a box of `width` px — the donor's `wrapper`, which happy-dom cannot lay out. */
...
describe('the sidebar boundary drags, within the donor clamps', () => {
  it('opens at the donor default', async () => {
  ...
  it('stands the transitions down for the duration of the gesture', async () => {
    // An eased width lags a pointer that is setting it directly (the donor's own reason for zeroing
    // its transition durations at pointer-down, ui/sidebar.tsx:433-435).
    const el = await mount();
    const grip = q(el, 'sv3-sidebar-grip');
    grip.setPointerCapture = (): void => undefined;
    grip.dispatchEvent(
  ...
  it('cannot go under the donor floor', async () => {
    const el = await mount();
    await drag(el, -5000);
...
describe('the icon rail', () => {
  ...
  it('keeps a compact row NAMED even though its title is not drawn', async () => {
    ...
    // The donor shows a right-side tooltip on a collapsed row (ui/sidebar.tsx:860-871); this window
...
describe('renaming a conversation from its row', () => {
  ...
  it('does not resurrect a cancelled edit through the blur that follows it', async () => {
    ...
    // blur would commit exactly the text the reader just discarded (donor `Sidebar.tsx:930-953`).
    const el = await mount();
    await ready(el);
    const input = await openRename(el);
    input.value = 'discarded';
