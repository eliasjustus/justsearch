...
/**
 * jf-sv3-empty — the Search v3 window's zero state (tempdoc 822 slice 4, donor §6.6).
 *
 * Derived from T3 Code (T3 Tools Inc., MIT) — see THIRD-PARTY-NOTICES.md in this directory.
 *
 * ONE element for every empty region, because the donor has one: the sidebar with no threads and the
 * content surface with no results are the same pattern at two sizes, so they must not drift into two.
 *
 * The media is the donor's fanned three-card stack — a bordered tile with two `aria-hidden` copies
 * behind it, rotated ±10° and scaled to 84% off their bottom corners. It is memorable and costs no
 * illustration asset, which is the whole reason the donor chose it. The two copies are decoration and
...
export class Sv3Empty extends JfElement {
  static styles = [
    ...
    css`
      ...
      /* The donor's one breakpoint on this component: a roomier region gets a roomier state. */
      ...
      /* The 1px inset ring the donor puts on every card-like tile: a hairline BELOW the edge in light,
      ...
      /* Individual transform properties rather than the shorthand: the donor composes translate,
