...
/**
 * jf-sv3-palette — the Search v3 window's command palette (tempdoc 822 slice 4, donor §3.3).
 *
 * Derived from T3 Code (T3 Tools Inc., MIT) — see THIRD-PARTY-NOTICES.md in this directory.
 *
 * WINDOW-SCOPED, not document-fixed. The donor's palette is a real `<dialog>` in the browser's top
 ...
 *
 * Two donor details are the point of the whole component:
 *
...
export class Sv3Palette extends JfElement {
  static styles = [
    ...
    css`
      ...
      /* Sized by the window host it covers, which makes the region ITSELF the palette's viewport —
         cqh below then means "a share of the window", the honest re-expression of the donor's vh
      ...
      /* The donor strips the field's own ring inside the palette, and live measurement showed why the
      ...
      /* ── The panel: the list's own clip, and the conditional bottom corners ──
         The donor makes the bottom corners follow the FOOTER's presence rather than a flag, so a
      ...
      /* The scroll fade is a MASK, never an overlay gradient: an overlay only works while the chrome
         ...
         donor's eased curve; the third layer keeps the scrollbar column at full opacity. */
      .list {
        flex: 1 1 auto;
        min-height: 0;
        padding: var(--space-2);
  ...
  /**
   * Focus may not leave an open palette. The donor's dialog gets this from the platform (`showModal`
