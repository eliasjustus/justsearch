 ...
 * it), focus cannot leave an open palette and returns to whatever opened it, and the two donor
...
describe('the palette is a window-scoped overlay, never a document one', () => {
  ...
  it('opens from the topbar with the donor anatomy: input, list, separator, footer', async () => {
    const el = await mount();
...
describe('Ctrl+K is scoped to the window, and the shipped binding is left alone', () => {
  it('opens the palette for a chord raised inside the window', async () => {
    ...
    await palette.updateComplete;
    expect(palette.open).toBe(true);
    // ...and toggles shut on the same chord, which is what the donor's trigger does.
    field?.dispatchEvent(chord());
    await palette.updateComplete;
    expect(palette.open).toBe(false);
  });
...
/**
 * Donor §3.3's two-state distinction, rendered: SELECTED is the current choice and HIGHLIGHTED is
...
describe('selection and highlight are two different states', () => {
  ...
  it('filters on the query and shows the donor empty row when nothing matches', async () => {
    const el = await mount();
    const { palette } = await openViaTopbar(el);
    const input = palette.shadowRoot?.querySelector('input');
    input!.value = 'reindex';
