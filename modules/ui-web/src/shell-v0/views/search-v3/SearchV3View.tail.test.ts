 ...
 * became one 24px row at the donor's own footer geometry. What is pinned here is not the appearance
...
describe('the disclosure is the window\'s own, per turn, and it opens the SHARED panel', () => {
  ...
  it('swaps the chevron rather than rotating one glyph — the donor\'s own inline disclosure', async () => {
    // The shared icon set's Lucide paths (`components/Icon.ts`), which is the only thing that tells
    // the two glyphs apart in the DOM. The imported panel ROTATES one glyph; the donor's inline
    // disclosure swaps two, and this window takes the donor's.
    ...
    // ...and the chevron is sized at the donor's 14px, one step up from the 12px text beside it.
...
describe('copy is the only yielding element, and it reports without renaming itself', () => {
  it('keeps its accessible name and announces through the row\'s live region', async () => {
    ...
    // The donor's glyph pair, told apart by their own shapes: `clipboard-copy` opens with a <rect>,
