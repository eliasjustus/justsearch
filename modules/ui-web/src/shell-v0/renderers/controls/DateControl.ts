 ...
 * and `format: "date"`. Rank: RANK_SPECIALIZED_CONTROL — outranks
 * the basic text tester so date-shaped schemas get a date picker.
 *
 * Wire format: ISO-8601 date string (YYYY-MM-DD), matching
 * `<input type="date">` value semantics.
 */
...
export class DateControl extends JsonFormsRendererBase {
  static styles = css`
    :host {
      display: block;
...
customElements.define('jf-date-control', DateControl);

export const dateControlTester: RendererTester = (
  ...
  if ((schema as { format?: string }).format !== 'date') {
    return -1;
  }
...
registerRenderer(dateControlTester, 'jf-date-control');
