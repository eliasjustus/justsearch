...
| # | Slug | Rule | Tier | Resolves to | Catches violations via |
|---|---|---|---|---|---|
| 27 | `subagents-no-inheritance` | Subagent CLAUDE.md inheritance is agent-type-dependent (gp/custom: full; Explore/Plan/fork: none — hook brief only); task-specific brief inline always | `prose-only` | — | Platform behavior (re-verified 2026-07-16, tempdoc 743 probes; supersedes the pre-2026-07 "no inheritance" claim). The `subagent-guide` SubagentStart hook injects the Hard-Invariants baseline for all types (sole carrier for Explore/Plan); the task-specific brief is honor-system |
