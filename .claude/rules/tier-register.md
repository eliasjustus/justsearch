...
| 18 | `delegating-to-subagents` | Delegating to Subagents — brief inline, no destructive git delegation | `prose-only` | — | Mostly agent discipline: brief-inline and no-destructive-git delegation cannot be mechanically inspected. Two subsets are now enforced, so the old "`bash-guard` gives **no** subagent coverage" claim no longer holds in general: (a) the **model-routing** subset — "set an explicit `model` on every subagent" — is hook-enforced by `subagent-model-guard.mjs` (PreToolUse/`Agent`), which blocks an unpinned or fable-family spawn; (b) it also bites on *nested* spawns, the live-verified exception to "parent hooks don't fire in subagents" (row 28; agent-lessons.md, 2026-07-12 probe). Unverified beyond PreToolUse/`Agent` — fork-type subagents ignore `model` (claude-code#74788) and the background dispatch-picker path (#64493) are uncovered, so the rest stays prose |
