...
**Behavior:** When you edit retrieval-engine source that can change ranking quality, performance, OR recall-survival (`modules/{adapters-lucene,reranker,worker-services,search,app-search}/src/**`, or `app-services/.../{worker,gpl}/`), this hook reminds you to re-run an eval + the **three** retrieval ratchets — the Q-010 relevance ratchet (nDCG@10), the tempdoc-640 performance ratchet (latency/throughput/footprint), and the tempdoc-636/D-005 recall-leak ratchet (cascade-leak rate — a leg's correct answer dropped before the judge). It ALSO fires on **inference-path** edits (`modules/{app-inference,prompt-support}/src/**`, `app-services/.../{conversation,inference}/`) — a distinct subject — nudging the tempdoc-640-L **LLM-generation-latency ratchet** (`jseval llm-bench` + `llm-gate`: TTFT / e2e / tokens-sec). It ALSO fires on **MCP-tool-surface** edits (`modules/ui/src/main/java/io/justsearch/ui/api/mcp/**`) — a third distinct subject: whether an LLM agent can still successfully DRIVE the retrieval tool, not raw retrieval quality — nudging the tempdoc-673 **agent-utility detection gate** (`utility-gate`: condition-C absolute-accuracy floor on the `util-smoke` fabricated corpus). Unlike the other two ratchet families, this one costs a real paid agent-call run, so the trigger set is deliberately narrower and the nudge frames it as deliberate/periodic, not auto-run. So a regression on any axis fails loudly rather than coasting (the engine's missing continuous-quality pressure). Self-filters by path; never blocks.
**Action:** Run a **full** measurement eval with the **leg modes** (the leak gate reads the `staged_recall_accounting` projection, which needs `vector,lexical,splade` + the hybrid final in one run; perf-gate also needs the cross-encoder + pipeline timeline): `jseval run --start-backend --clean --pipeline --ce --embedding --splade --dataset scifact --modes vector,lexical,splade,hybrid`, then `python -m jseval relevance-gate --data-dir <dir> --dataset beir/scifact`, `python -m jseval perf-gate --data-dir <dir> --dataset scifact`, **and** `python -m jseval leak-gate --data-dir <dir> --dataset beir/scifact`. (Relevance + leak key on `beir/scifact`; perf + the run use the raw slug `scifact` — all intended.) Floors live in `scripts/jseval/{relevance,perf}-ratchet-baselines.v1.json` + `leak-gate-baselines.v1.json`; re-pin perf with `perf-gate --update-baseline`, leak with `leak-gate-derive --datasets <slugs>`. For an MCP-surface edit, run `python -m jseval utility-gate --record <utility-comparison.v1.json> --corpus golden/util-smoke` (records come from a live `agent-eval`/`utility-compose` against `util-smoke/`, condition C only — see `scripts/jseval/util-smoke/README.md`; only a fabricated/engineered corpus can be pinned here — tempdoc 673 D8 refuses a realistic one by default); re-pin with `utility-gate --update-baseline`. Floor: `scripts/jseval/utility-ratchet-baselines.v1.json`. Load `/jseval` for flags. (Relevance-gate checks nDCG@10 *mean* regression — distinct from `jseval gate`, nDCG@10 *stdev* drift; perf-gate checks relative ratio bands; leak-gate checks the cross-mode `leak_rate` against a measured ceiling+tolerance; utility-gate checks the condition-C absolute floor against a measured floor−tolerance, downgrading to INCONCLUSIVE — tempdoc 673 D9 — instead of a false FAIL when the agent's cli_version drifted since the baseline was pinned.)
