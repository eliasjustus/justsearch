...
- **`--llm` cold start** may fail once: Worker port discovery (15s timeout) races with GGUF model load (~5GB disk read). Retry succeeds because OS file cache is warm. Not a code bug.
