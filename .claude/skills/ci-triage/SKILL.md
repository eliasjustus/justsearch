...
## Manual Workflow Signals

Before interpreting a manually dispatched GitHub Actions failure, run:

```bash
node scripts/ci/workflow-signal-health.mjs --repo eliasjustus/justsearch --md
```

Use the computed failure class to route the five active manual workflow files: `ci.yml`, `docs-lint.yml`, `build-installer.yml`, `codeql.yml`, and `phase-3-observability-nightly.yml`.

- `release-blocking-failure` on `Build Installer` routes to tempdoc 374 / Production-Reality Verification.
- `workflow-assumption-drift` on the CI `Stress suite coverage gate` means `scripts/ci/stress-suite-policy.v1.json` is stale; update the policy before interpreting the stress lane as product evidence.
- `product-regression` on CI `Run stress tests` means a policy-covered `@Tag("stress")` suite failed; route to the owning subsystem.
...
After touching any `@Tag("stress")` subject or concurrency-sensitive code, dispatch:
