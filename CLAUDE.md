...
**Model routing (delegation economics; owner decisions 2026-07-07, updated 2026-07-14).** This paragraph binds the ORCHESTRATOR — whatever model runs the main loop (Fable or Opus alike). **Orchestrator tokens are the scarcest resource (owner, 2026-07-14): prefer even INEFFICIENT delegation over inline execution.** A worker re-deriving context the orchestrator already holds is an acceptable cost; the orchestrator spending its own tokens on mechanical work is not — when unsure whether delegation pays, delegate anyway. Orchestration — decomposition, briefs, design, judging returned evidence — is the main loop's job; implementation is token-heavy, so delegate bounded, verifiable chunks. **Delegate mechanical work by default once it is enumerable** — when a diagnosis ends and the remaining work is a known list, bundle it into a worker brief with self-verifying acceptance criteria instead of continuing inline (owner directive 2026-07-14); do not pull worker-grade loops into the main loop out of quality doubt. Pilot exception (P-C, 743, owner-approved 2026-07-17): a chunk clearly below the spawn break-even (brief-writing + worker re-orientation + notification round-trip would exceed the task) is done directly — estimate first, log the call in the tempdoc; threshold derived locally by the pilot, and when unsure, still delegate. **Set an explicit `model` on every subagent** — an unset `model` inherits the parent, silently billing orchestrator-tier for worker-grade work. Sonnet is the floor for findings you'll rely on; pin `model: "opus"` where sonnet quality is in question (opus workers are capable enough that main-loop inlining is not the answer); haiku only where wrong output is self-evident. Never delegate: brief-writing and evidence judgment, main-checkout writes, merge/publish, irreversible actions, trivial edits (single-command scale only — an edit+test+doc bundle is already delegable). Dev-stack (owner decision 2026-07-14, supervised form): lease acquisition/takeover/teardown and contention decisions stay main-loop; the orchestrator (Fable or Opus main loop) MAY delegate stack-driving work inside a window it has already leased and actively supervises (Monitor on logs/markers; the orchestrator remains responsible for stopping the run), and the brief must inline the contention rules a subagent cannot inherit. **Fire-and-forget stack delegation is never allowed** — that is the predictable evasion, not a variant. Chunk long refactors into bounded delegations. If a worker's output misses the bar, redo it with a stronger model without asking — judge the output, not the price tag. Falsifier: cost-per-shipped-merge should improve within ~2 months without rework rising; flat → delete this paragraph; rework up → raise the floor.
