...
def test_historical_fixture_semantic_digest_repinned_after_624_itt_change():
    ...
    Re-pinned AGAIN 2026-07-28 (v3 AMENDMENT 1, tempdoc 782 §E.0 / §H — the
    SAME re-pin class, 5th occurrence): the active policy's
    `required_schema_strata.known_schemas` narrowed from `["1_hop", "2_hop"]`
    to `["1_hop"]`, a corpus-description correction (the certified hero corpora
    are 100% `1_hop` by construction, so requiring a `2_hop` breakdown made
    every hero stratum unpromotable). No threshold and no stratum changed.
...
def test_amendment_1_repin_is_policy_identity_only_prior_pin_reproduces():
    """Amendment 1's re-pin (2026-07-28, tempdoc 782 §H BLOCKER-1) is legitimate
    only if the AMENDED POLICY BYTES moved the digest and nothing else did.
    ...
    never any measurement content, so the digest may move through `policy_hash`
    ...
    schema_spec["_note"] = schema_spec["_note"].split(" AMENDMENT 1")[0]
    assert pre_amendment["policy_changelog"][1]["amendment"] == 1
    pre_amendment["policy_changelog"] = pre_amendment["policy_changelog"][:1]

    record = finalize_evidence(
        [path], composed_at="fixture", policy=pre_amendment)
    assert record["semantic_digest"] == (
