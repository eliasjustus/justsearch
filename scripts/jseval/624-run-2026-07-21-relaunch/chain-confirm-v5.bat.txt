...
rem ===========================================================================
rem  RELAUNCH campaign chain v5 (tempdoc 624 Relaunch pre-registration
rem  2026-07-21; supersedes chain-confirm.bat/v4). Target: accepted/adoption-only
rem  under the RATIFIED rate-based policy. LAUNCH GATES (both required, founder):
rem  (1) the v2 amendment is ratified into utility-claim-policy.v1.json;
rem  (2) explicit launch authorization. DO NOT run before both.
rem  v5 deltas vs v4: warm/adopt reinstated (751 sec-Q fixed, PR #256);
rem  serve_up adopts via --index-cache-mode; calibrations are git_sha+CLI
rem  stamped and fail closed on drift (758); DISABLE_AUTOUPDATER pins the
rem  harness window; root mode now enforces corpus-dir derivation (#258).
rem  Claim policy is ACTIVE, the EXACT four-stratum matrix is required, no
rem  partial-value ladder.
rem
...
rem --- WARM step REINSTATED (v5, 2026-07-21): the 751 sec-Q wedge is fixed and
rem     live-verified (PR #256): warm skips ingest on adopt - single ingest pass -
rem     and ingest_and_wait uses the union floor for an already-watched root, so
rem     the chain topology serve-adopt + separate-process ingest cannot wedge.
rem     Warm is build-or-adopt + publish; serve_up then adopts in seconds.
...
rem     (ingest stays as the idempotent safety net; union floor makes it a no-op
rem     against the adopted index)
...
start "serve-%~1" /b cmd /c "python serve-eval-backend.py --port %PORT% --index-cache-mode on --corpus-dir "%~3" --ready-file "!RDY!" --stop-file "!STOP!" --stopped-file "!STOPPED!" --failed-file "!FAILED!" > "%~2\serve.log" 2>&1"
