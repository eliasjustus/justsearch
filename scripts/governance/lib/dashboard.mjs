 ...
 *
 * Reads committed governance authorities and emits
 * `docs/reference/governance-state.md`, a GitHub-renderable summary of the
 * gate roster and discipline-debt totals. Timestamped local run history is
 * written only to the gitignored `tmp/governance-runs.md` side-file.
 *
...
import { readGateHistory, readRepositoryHealthHistory } from './history.mjs';
 ...
 * baselines, never a fork. The committed human markdown follows the same boundary.
 */
...
function generate() {
  ...
  lines.push(`description: "Stable gate roster + committed discipline-debt totals. Regenerated by scripts/governance/lib/dashboard.mjs."`);
  lines.push(`---`);
  lines.push('');
  ...
  lines.push('');
  lines.push('| Gate | Title | Tier | Changesets |');
  lines.push('|---|---|---|---|');
  for (const gate of reg.gates ?? []) {
    const tier = gate.baseline?.kind === 'ratchet-file' ? 'ratchet' : gate.changesetsDir ? 'changeset-gated' : 'scan';
    lines.push(`| \`${gate.id}\` | ${gate.title ?? ''} | ${tier} | ${gate.changesetsDir ? 'yes' : 'no'} |`);
  }
  lines.push('');
...
function generateRuns() {
  const history = readGateHistory({ repoRoot: REPO_ROOT });
  const healthHistory = readRepositoryHealthHistory({ repoRoot: REPO_ROOT });
  const out = [];
  ...
  out.push('');
  if (history.length === 0 && healthHistory.length === 0) {
  ...
  const latestHealth = healthHistory.at(-1);
  if (latestHealth) {
    const metrics = latestHealth.metrics ?? {};
    const locByModule = metrics.productionSourceLocByModule ?? {};
    const totalLoc = Object.values(locByModule).reduce((sum, value) => sum + Number(value || 0), 0);
    out.push('');
    out.push('## Latest local repository-health snapshot');
    out.push('');
    out.push(`Captured ${latestHealth.ts ?? 'unknown'} from the same kernel invocation as its gate rows.`);
    out.push('');
    out.push(`- Gradle modules: **${metrics.gradleModuleCount ?? 0}**`);
    out.push(`- Test source files: **${metrics.testFileCount ?? 0}**`);
    out.push(`- Production source LOC: **${totalLoc}** across ${Object.keys(locByModule).length} module directories`);
    out.push('');
    out.push('This snapshot is local and retention-bounded; it is not a cross-machine durable history.');
  }
  return out.join('\n') + '\n';
}
...
function main() {
  ...
  if (args.check) {
    // Only the STABLE machine projection is freshness-gated. The markdown may summarize local gate
    // history and is intentionally not a check subject; timestamped health stays under tmp/ only.
