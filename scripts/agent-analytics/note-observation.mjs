...
export function renderRouting(description) {
  return [
    ...
    '  red/flaky verification command on main    -> pin with an exit in scripts/agent-analytics/expected-state.v1.json',
    '                                               (known-state-hint delivers it) + the fix as a tracked item',
