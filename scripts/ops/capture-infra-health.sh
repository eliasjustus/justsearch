...
mkdir -p "$SUMMARY_DIR"
TS="$(date +%Y%m%d-%H%M%S)"
RUN_LOG="$SUMMARY_DIR/capture-infra-health-$TS.log"
...
finalize_evidence() {
  ...
  local cap_args=(
    ...
    --scenario=capture-infra-health
    --api-base-url=none
    --out-root=tmp/agent-evidence
    --trace=false
    --attach-label=infra_health
    --attach-file="$RUN_LOG"
    --external-status="$ext_status"
  )
  if [[ "$ext_status" == "failed" ]]; then
    cap_args+=(--external-error="capture-infra-health failed (exit=$exit_code)")
  fi
  if [[ -d "$REPO_ROOT/reports/phase7/health" ]]; then
    cap_args+=(--attach-dir="$REPO_ROOT/reports/phase7/health")
  fi
...
echo "[capture-infra-health] Running integration test to regenerate reports/phase7/health" >&2
./gradlew :modules:app-services:integrationTest --tests '*InfraHealthDiagnosticsIntegrationTest'

echo "[capture-infra-health] Completed. See reports/phase7/health for payloads." >&2
