...
$gradleStdout = Join-Path -Path $summaryDir -ChildPath ("launcher-reindex-jni-" + $timestamp + ".stdout.log")
$gradleStderr = Join-Path -Path $summaryDir -ChildPath ("launcher-reindex-jni-" + $timestamp + ".stderr.log")

$scenario = "launcher-reindex-jni"
$bundleDir = $null
$exitCode = 0
...
try {
  if ($LlamaJniPath) {
    ...
    $env:JAVA_TOOL_OPTIONS = '-Dllm.backend=jni -Dllama.jni.lib.path="' + $LlamaJniPath + '" -Djustsearch.ci=false'
  }

  $p = Start-Process -FilePath (Join-Path -Path $repoRoot -ChildPath "gradlew.bat") `
    -ArgumentList @("--no-daemon", ":modules:app-launcher:run", "--args=reindex") `
    -NoNewWindow -Wait -PassThru `
    -RedirectStandardOutput $gradleStdout `
    -RedirectStandardError $gradleStderr
  $exitCode = $p.ExitCode
  if ($exitCode -ne 0) {
    throw "launcher reindex (jni) failed (exit=$exitCode)"
  }
} catch {
