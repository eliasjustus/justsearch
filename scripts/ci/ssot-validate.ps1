...
$runStdout = Join-Path -Path $summaryDir -ChildPath ("ssot-validate-" + $timestamp + ".stdout.log")
$runStderr = Join-Path -Path $summaryDir -ChildPath ("ssot-validate-" + $timestamp + ".stderr.log")

$scenario = "ssot-validate"
$exitCode = 0
$errMsg = $null
$bundleDir = $null
...
try {
  ...
  $p2 = Start-Process -FilePath "node" -ArgumentList @("validate.mjs") -NoNewWindow -Wait -PassThru -RedirectStandardOutput $runStdout -RedirectStandardError $runStderr
  $exitCode = $p2.ExitCode
  if ($exitCode -ne 0) { throw "ssot validate failed (exit=$exitCode)" }
} catch {
