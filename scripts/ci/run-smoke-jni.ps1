Param(
  [Parameter(Mandatory=$true)][string]$LlamaJniPath,
...
if (-not (Test-Path $LlamaJniPath)) { throw "JNI path not found: $LlamaJniPath" }
...
$gradleStdout = Join-Path -Path $summaryDir -ChildPath ("launcher-smoke-jni-" + $timestamp + ".stdout.log")
$gradleStderr = Join-Path -Path $summaryDir -ChildPath ("launcher-smoke-jni-" + $timestamp + ".stderr.log")

$scenario = "launcher-smoke-jni"
$bundleDir = $null
$exitCode = 0

try {
  # Quote the JNI path to tolerate spaces (PowerShell escaping: build via concatenation).
  $env:JAVA_TOOL_OPTIONS = '-Degress.block_all=true -Djustsearch.ci=true -Dllm.backend=jni -Dllama.jni.lib.path="' + $LlamaJniPath + '" -Dprism.order=sw'

  $p = Start-Process -FilePath (Join-Path -Path $repoRoot -ChildPath "gradlew.bat") `
    -ArgumentList @("--no-daemon", ":modules:app-launcher:run", "--args=--smoke") `
    -NoNewWindow -Wait -PassThru `
    -RedirectStandardOutput $gradleStdout `
    -RedirectStandardError $gradleStderr
  $exitCode = $p.ExitCode
  if ($exitCode -ne 0) {
    throw "launcher smoke (jni) failed (exit=$exitCode)"
  }
} catch {
