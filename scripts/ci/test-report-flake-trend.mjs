...
// ---------------------------------------------------------------------------
{
  ...
  assert.match(atThreshold.warnings[0], /note-observation\.mjs/, 'warning suggests the inbox helper, does not write itself');
...
// buildSuggestedObservationText is a pure formatter — spot-check independently of aggregation.
{
  ...
  assert.match(text, /^node scripts\/agent-analytics\/note-observation\.mjs "/);
