...
import { firstSchemaDifference } from './check-shape-handler-live.mjs';
...
if (failures.length > 0) {
  console.error(`check-shape-handler-live.test FAILED (${failures.length}):`);
  for (const failure of failures) console.error(`  - ${failure}`);
  process.exit(1);
}

console.log(`check-shape-handler-live.test OK - ${passed} assertions passed.`);
